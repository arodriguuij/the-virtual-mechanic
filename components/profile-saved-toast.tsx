 ...
 * `profile_error`/`strava_error`), but unlike those persistent warning
 * banners this is a self-dismissing toast — it fades out and strips the
 * query param after a few seconds so a page refresh doesn't keep re-showing
 * a save confirmation for an action that already happened.
 */
export function ProfileSavedToast() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex items-center gap-2 border border-status-good/30 bg-status-good/10 px-4 py-3 text-sm text-status-good shadow-sm">
      ...
      Perfil guardado correctamente en Supabase
    </div>
  );
}
